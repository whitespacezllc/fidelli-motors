-- ============================================================
-- Fidelli Motors · La experiencia gana dos colores: fondo y papel
--
-- Pedido de Brothers: más libertad para personalizar lo que ve el
-- cliente. Ya existían el color de acento (color_primario), el logo y
-- los campos visibles; se suman:
--
--   · color_fondo   → el fondo de la página pública entera
--   · color_carton  → el papel del cartón (la tarjeta)
--
-- Los dos son NULLABLES: null = el blanco de siempre. No hay backfill
-- que hacer — ningún tenant cambia de aspecto por esta migración.
--
-- LA REGLA DE LEGIBILIDAD vive en la app (action + form): solo tonos
-- CLAROS (luminancia WCAG ≥ 0.7). El texto de la landing es tinta
-- oscura fija y Pedro la lee al sol — un fondo oscuro la vuelve
-- ilegible. El CHECK de acá abajo solo garantiza que sea un hex: lo
-- que entra a un atributo style tiene que ser un color y nada más.
-- ============================================================

alter table config_experiencia add column color_fondo text;
alter table config_experiencia add column color_carton text;

alter table config_experiencia add constraint color_fondo_hex
  check (color_fondo is null or color_fondo ~ '^#[0-9A-Fa-f]{6}$');
alter table config_experiencia add constraint color_carton_hex
  check (color_carton is null or color_carton ~ '^#[0-9A-Fa-f]{6}$');

comment on column config_experiencia.color_fondo is
  'Fondo de la página pública. null = blanco. Solo tonos claros (la app exige luminancia ≥ 0.7).';
comment on column config_experiencia.color_carton is
  'El papel del cartón digital. null = blanco. Misma regla de claridad que color_fondo.';


-- ---------- get_landing: el shell viaja con los colores ----------
...
comment on function get_landing is
  'Shell público de la landing: marca, colores, contacto de la marca y sucursales activas. No escribe.';


-- ---------- get_carton: ídem en sus dos ramas ----------
-- Misma función de 20260728130000 (con `cambiado` en los items); el
-- bloque del lubricentro suma los dos colores en la rama encontrada y
-- en la de patente_no_encontrada — esa pantalla también se pinta.

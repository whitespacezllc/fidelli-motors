#!/bin/bash
# La rotura a mano de R17 (regla 13): cada bloque se corre con SU rotura
# —la regla exacta que dice cubrir— y tiene que ponerse en ROJO. Una
# prueba que nunca se vio fallar es una prueba que no existe.
#
...
correr() { # $1 = nombre · $2 = SQL de la rotura · $3 = bloque (R17x) · $4 = patrón esperado en el error
  local salida
  salida=$( { echo "begin;"; echo "$2"; bloque "$3"; echo "rollback;"; } | $DB -f - 2>&1 )
